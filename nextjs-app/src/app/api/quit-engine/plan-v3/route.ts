...
type V3Answers = EngineAnswers & { personal_plan_v2: PersonalPlanV2Answers; adaptive_assessment: AdaptiveAssessmentAnswers }
type V3Result = EngineResult & { personal_plan_v2: PersonalPlanV2Enrichment; adaptive_triage: AdaptiveTriageProfile }
type V3Plan = Omit<StoredQuitPlan, 'answers' | 'result'> & { answers: V3Answers; result: V3Result }
...
export async function POST(request: NextRequest) {
  ...
  try { answers = validateEngineAnswers(rawBase) } catch (error) {
    console.warn('Aqla v3 base validation failed', error instanceof Error ? error.message : 'unknown')
  ...
  try { v2Answers = validatePersonalPlanV2Answers(body.personal_plan_v2, answers) } catch (error) {
    console.warn('Aqla v3 personal-plan validation failed', error instanceof Error ? error.message : 'unknown')
  ...
  const result = buildPlan(answers, lang) as V3Result
  ...
  if (!result.safety_immediate) {
    try {
      ...
      const ai = await openAIStructuredResponse<Personalisation>({
        schemaName: 'aqla_adaptive_quit_plan_v3',
        schema,
        maxOutputTokens: 1100,
        instructions: `You personalise the coaching layer of Aqla, a Saudi smoking and nicotine cessation support platform. Deterministic Aqla logic is authoritative for safety, referral, triage, scoring and instrument interpretation. Never recalculate or contradict the supplied deterministic_triage. PSECDI and HSI results may be described as screening/dependence indicators, not diagnoses. Any AQla oral-nicotine/pouch screen is explicitly non-validated and must never be described as validated. Do not diagnose, prescribe, choose medication doses, promise outcomes or invent clinical claims. Use ${lang === 'ar' ? 'clear modern Arabic suitable for Saudi users' : 'clear British English'}. Personalise specifically for the supplied product pattern: cigarettes, vaping, nicotine pouches, shisha or mixed use. If products are mixed, prioritise the supplied primary product and substitution pattern rather than pretending all products are equivalent. Use followup_focus and support_need to make the next actions practical. Do not mention AI. Do not use financial spending values; they are deliberately withheld. Return only the requested JSON schema.`,
      ...
      console.error('Aqla adaptive AI personalisation unavailable', error instanceof Error ? error.message : 'unknown')
