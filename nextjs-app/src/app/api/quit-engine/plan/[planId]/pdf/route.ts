...
import { renderQuitPlanPdf } from '@/lib/quit-engine/pdf.server'
...
export async function GET(
  ...
  try {
    const pdf = await renderQuitPlanPdf(plan, lang)
    const filename = lang === 'ar' ? `aqla-plan-${planId}-ar.pdf` : `aqla-plan-${planId}.pdf`
    ...
    console.error('Aqla server PDF generation failed', error instanceof Error ? error.message : 'unknown')
