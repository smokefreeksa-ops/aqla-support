...
export async function getCommunityStats(){
 ...
 const visibleCities:Array<{city:string;engagement_count:number;challenge_joins:number;volunteer_applications:number}>=[];let hiddenEngagement=0
 for(const row of cityRows.Items??[]){const count=Number(row.engagement_count||0);if(count>=CITY_PUBLIC_MIN_CELL)visibleCities.push({city:String(row.city||''),engagement_count:count,challenge_joins:Number(row.challenge_joins||0),volunteer_applications:Number(row.volunteer_applications||0)});else hiddenEngagement+=count}
 visibleCities.sort((a,b)=>b.engagement_count-a.engagement_count)
 return {generated_at:new Date().toISOString(),minimum_city_cell:CITY_PUBLIC_MIN_CELL,totals:{challenge_joins:Number(totals.Item?.challenge_joins||0),challenge_completions:Number(totals.Item?.challenge_completions||0),points_awarded:Number(totals.Item?.points_awarded||0),volunteer_applications:Number(totals.Item?.volunteer_applications||0),cities_visible:visibleCities.length},challenges:(challengeRows.Items??[]).map(row=>({challenge_id:String(row.SK).replace('CHALLENGE#',''),joined_count:Number(row.joined_count||0),completed_count:Number(row.completed_count||0),points_awarded:Number(row.points_awarded||0)})),cities:visibleCities,other_small_cell_engagement:hiddenEngagement}
}
