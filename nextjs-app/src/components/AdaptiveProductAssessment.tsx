...
function Question({ title, hint, children }: { title: string; hint?: string; children: React.ReactNode }) {
  return <div className="qe-question-block"><h3>{title}</h3>{hint ? <p>{hint}</p> : null}{children}</div>
}
...
export function adaptiveProductQuestionsComplete(base: EngineAnswers, value: AdaptiveAssessmentAnswers) {
  ...
  if (base.product_types.includes('vape')) {
    ...
    if (v.hard_to_quit === undefined || v.strong_cravings === undefined || !v.urge_strength || v.hard_to_refrain_where_not_allowed === undefined || v.irritable_when_unable === undefined || v.nervous_restless_anxious_when_unable === undefined) return false
  }
  if (base.product_types.includes('pouches')) {
    const p = value.pouches
    if (p?.pouches_per_day === undefined || p.night_use === undefined || p.strong_cravings === undefined || p.hard_to_cut_down === undefined || p.irritable_when_unable === undefined || p.uses_more_than_intended === undefined || p.uses_multiple_at_once === undefined || p.changes_strength === undefined) return false
  }
  return true
}
...
export default function AdaptiveProductAssessment({ lang, base, value, onChange }: Props) {
  ...
  return <div className="qe-stack">
    ...
    {base.product_types.includes('vape') ? <div className="qe-stack">
      ...
      {value.vape?.awakens_at_night ? <Question title={ar ? 'في كم ليلة من الأسبوع يحدث ذلك عادةً؟' : 'On how many nights per week does that usually happen?'}><input type="number" min="0" max="7" step="1" value={value.vape?.nights_per_week ?? ''} onChange={(event) => patchVape({ nights_per_week: event.target.value ? Number(event.target.value) : undefined })} /></Question> : null}
      ...
      <Question title={ar ? 'اختياري: تركيز النيكوتين المكتوب على السائل أو الجهاز (mg/mL)' : 'Optional: nicotine strength shown on the liquid/device (mg/mL)'}><input type="number" min="0" max="100" step="0.1" value={value.vape?.nicotine_strength_mg_ml ?? ''} onChange={(event) => patchVape({ nicotine_strength_mg_ml: event.target.value ? Number(event.target.value) : undefined })} /></Question>
    </div> : null}
    ...
    {base.product_types.includes('pouches') ? <div className="qe-stack">
      ...
      <Question title={ar ? 'كم كيس نيكوتين تستخدم تقريبًا في اليوم؟' : 'About how many nicotine pouches do you use per day?'}><input type="number" min="0" max="100" step="1" value={value.pouches?.pouches_per_day ?? ''} onChange={(event) => patchPouch({ pouches_per_day: event.target.value ? Number(event.target.value) : undefined })} /></Question>
      <Question title={ar ? 'اختياري: ما اسم المنتج أو العلامة؟' : 'Optional: what product or brand do you use?'}><input maxLength={80} value={value.pouches?.brand ?? ''} onChange={(event) => patchPouch({ brand: event.target.value })} /></Question>
      <Question title={ar ? 'اختياري: كم ملغ نيكوتين في الكيس الواحد؟' : 'Optional: how many mg of nicotine per pouch?'}><input type="number" min="0" max="100" step="0.1" value={value.pouches?.strength_mg_per_pouch ?? ''} onChange={(event) => patchPouch({ strength_mg_per_pouch: event.target.value ? Number(event.target.value) : undefined })} /></Question>
