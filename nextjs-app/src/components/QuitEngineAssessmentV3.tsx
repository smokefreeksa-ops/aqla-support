...
function RequiredLabel({ children }: { children: React.ReactNode }) {
  return <span>{children} <RequiredMark /></span>
}
...
function Question({ title, children }: { title: string; children: React.ReactNode }) { return <div className="qe-question-block"><h3>{title} <RequiredMark /></h3>{children}</div> }
...
export default function QuitEngineAssessmentV3({ signedIn }: { signedIn: boolean }) {
  ...
  const canNext = useMemo(() => {
    ...
    if (step === 7) {
      ...
      if (!answers.user_name?.trim() || !answers.support_person_name?.trim() || !v2.support_person_relationship?.trim()) return false
  ...
  return <main className="qe-page" dir={ar ? 'rtl' : 'ltr'} lang={lang}>
    ...
    <div className="qe-shell">
      <section className="qe-intro"><span className="qe-kicker">{ar ? 'أقلع — التقييم التكيفي الذكي' : 'Aqla — adaptive intelligent assessment'}</span><h1>{ar ? 'خطة تفهم نوع النيكوتين الذي تستخدمه' : 'A plan that understands the nicotine products you use'}</h1><p>{ar ? 'يبقى المسار 8 خطوات، لكن الأسئلة داخلها تتغير حسب السجائر أو الفيب أو أكياس النيكوتين أو الاستخدام المختلط.' : 'The journey stays at eight stages, while questions branch for cigarettes, vaping, nicotine pouches and mixed use.'}</p><div className="qe-required-legend"><RequiredMark /> {ar ? 'جميع أسئلة التقييم الظاهرة مطلوبة قبل المتابعة.' : 'All visible assessment questions are required before you continue.'}</div>{!signedIn ? <div className="qe-note success">{ar ? 'يمكنك إكمال الخطة كضيف دون تسجيل. أنشئ حسابًا فقط إذا أردت حفظها على المدى الطويل أو استلام البريد والمتابعة.' : 'You can complete the plan as a guest without registering. Create an account only if you want long-term saving, email and follow-up.'}</div> : null}<div className="qe-note">{ar ? 'تنبيه: أقلع أداة دعم رقمية قيد التطوير وليست بديلاً عن التقييم أو المشورة الطبية. قد يخطئ النظام أو تكون بعض النتائج غير مكتملة. لا تبدأ دواءً أو توقفه أو تغيّر جرعته اعتمادًا على أقلع دون استشارة مختص صحي مؤهل.' : 'Notice: Aqla is a digital support tool under development and is not a substitute for medical assessment or advice. The system can make mistakes and some results may be incomplete. Do not start, stop or change a medicine or dose based on Aqla without advice from a qualified healthcare professional.'}</div></section>
      ...
      <section className="qe-card">
        ...
        {step === 1 && <div className="qe-stack"><h2>{ar ? 'نمط الاستخدام الخاص بك' : 'Your product-specific pattern'}</h2>{!relapseOnly ? <Question title={ar ? 'بعد الاستيقاظ، متى تستخدم أول نيكوتين؟' : 'After waking, when do you first use nicotine?'}><SinglePick options={FIRST_USE_OPTIONS} value={answers.first_use_after_waking} lang={lang} onChange={(value: FirstUseAfterWaking) => update({ first_use_after_waking: value })} /></Question> : <div className="qe-note success">{ar ? 'أنت في مسار الحفاظ على الامتناع؛ لا نحتاج أسئلة كمية الآن.' : 'You are on the abstinence-maintenance pathway; quantity questions are not needed.'}</div>}
          {answers.product_types.includes('cigarettes') ? <><Question title={ar ? 'ما نوع السجائر؟' : 'What type of cigarettes?'}><SinglePick options={cigaretteForms} value={v2.cigarette_form} lang={lang} onChange={(value: CigaretteForm) => updateV2({ cigarette_form: value })} /></Question><Question title={ar ? 'كم سيجارة تستخدم في المتوسط؟' : 'How many cigarettes do you use on average?'}><div className="qe-input-grid"><label><RequiredLabel>{ar ? 'العدد' : 'Number'}</RequiredLabel><input aria-required="true" required type="number" min="0.1" max="500" step="1" placeholder={ar ? 'مثال: 20' : 'Example: 20'} value={v2.cigarette_quantity ?? ''} onChange={(event) => updateV2({ cigarette_quantity: event.target.value ? Number(event.target.value) : undefined })} /></label><label><RequiredLabel>{ar ? 'الفترة' : 'Period'}</RequiredLabel><select aria-required="true" required value={v2.cigarette_quantity_period ?? ''} onChange={(event) => updateV2({ cigarette_quantity_period: (event.target.value || undefined) as CigaretteQuantityPeriod | undefined })}><option value="">{ar ? 'اختر الفترة' : 'Choose period'}</option>{quantityPeriods.map((item) => <option key={item.value} value={item.value}>{item[lang]}</option>)}</select></label></div></Question></> : null}
          ...
          {!relapseOnly ? <Question title={ar ? 'كم تنفق على جميع منتجات النيكوتين؟' : 'How much do you spend on all nicotine products?'}><div className="qe-input-grid"><label><RequiredLabel>{ar ? 'المبلغ بالريال' : 'Amount in SAR'}</RequiredLabel><input aria-required="true" required type="number" min="0" max="100000" step="0.01" placeholder={ar ? 'مثال: 100' : 'Example: 100'} value={v2.nicotine_spend_amount ?? ''} onChange={(event) => updateV2({ nicotine_spend_amount: event.target.value ? Number(event.target.value) : undefined })} /></label><label><RequiredLabel>{ar ? 'الفترة' : 'Period'}</RequiredLabel><select aria-required="true" required value={v2.nicotine_spend_period ?? ''} onChange={(event) => updateV2({ nicotine_spend_period: (event.target.value || undefined) as SpendPeriod | undefined })}><option value="">{ar ? 'اختر الفترة' : 'Choose period'}</option>{spendPeriods.map((item) => <option key={item.value} value={item.value}>{item[lang]}</option>)}</select></label></div></Question> : null}
        </div>}

        {step === 2 && <div className="qe-stack"><h2>{ar ? 'المحفزات والروتين' : 'Triggers and routines'}</h2><Question title={ar ? 'اختر المحفزات التي تنطبق عليك' : 'Select triggers that apply to you'}><MultiPick options={TRIGGER_OPTIONS} values={answers.triggers} lang={lang} onToggle={(value: TriggerKey) => update({ triggers: toggle(answers.triggers, value) })} /></Question><Question title={ar ? 'هل يوجد شيء إضافي؟' : 'Anything else?'}><MultiPick options={ADDITIONAL_TRIGGER_OPTIONS} values={v2.additional_triggers} lang={lang} onToggle={(value: AdditionalTrigger) => updateV2({ additional_triggers: toggle(v2.additional_triggers, value) })} /></Question>{v2.additional_triggers.includes('other') ? <div className="qe-question-block"><h3>{ar ? 'المحفز الآخر' : 'Other trigger'} <RequiredMark /></h3><input aria-required="true" required maxLength={160} placeholder={ar ? 'اكتب المحفز الآخر' : 'Enter the other trigger'} value={v2.other_trigger_text ?? ''} onChange={(event) => updateV2({ other_trigger_text: event.target.value })} /></div> : null}</div>}
