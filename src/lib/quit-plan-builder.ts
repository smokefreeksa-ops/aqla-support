...
const TRIGGER_ACTIONS: Record<string, string> = {
  "مع القهوة": "غيّر مكان شرب القهوة أو أخّرها 10 دقائق، واجعل يدك مشغولة بكوب ماء أو علكة خالية من السكر.", "القهوة": "غيّر مكان شرب القهوة أو أخّرها 10 دقائق، واجعل يدك مشغولة بكوب ماء أو علكة خالية من السكر.", "التوتر": "استخدم قاعدة 10 دقائق: تنفس ببطء، اشرب ماء، غيّر المكان، ثم قرر بعد أن تهدأ الرغبة.", "بعد الأكل": "قم مباشرة بعد الأكل، اغسل أسنانك، أو امشِ 5 دقائق قبل أن تجلس.", "السيارة": "نظّف السيارة من أي بقايا، ضع علكة/ماء بجانبك، وغيّر مسارك المعتاد لأول أسبوع.", "العمل": "اربط الاستراحة بنشاط بديل (مشي قصير، اتصال، ماء) بدل الخروج للتدخين.", "الدراسة": "حدّد فترات تركيز قصيرة (25 دقيقة) ثم استراحة بنشاط حركي بدل النيكوتين.", "التجمعات": "أخبر شخصًا بقرارك قبل التجمع، اجلس بعيدًا عن المدخنين، وخطّط لمغادرة مبكرة لو زادت الرغبة.", "الأصدقاء المدخنون": "أخبر شخصًا بقرارك قبل التجمع، اجلس بعيدًا عن المدخنين، وخطّط لمغادرة مبكرة لو زادت الرغبة.", "الفراغ": "جهّز قائمة قصيرة من 5 أنشطة بديلة (مشي، قراءة، مكالمة، رياضة خفيفة، ترتيب) واستخدمها فورًا.", "الملل": "جهّز قائمة قصيرة من 5 أنشطة بديلة (مشي، قراءة، مكالمة، رياضة خفيفة، ترتيب) واستخدمها فورًا.", "قبل النوم": "غيّر روتين ما قبل النوم: دش دافئ، قراءة، أو تنفس بطيء بدل التدخين.", "السهر": "قلّل وقت السهر تدريجيًا، وغيّر بيئة السهر بعيدًا عن المثيرات أول أسبوعين.", "أول استخدام بعد الاستيقاظ": "أخّر أول رغبة 15 دقيقة بعد الاستيقاظ، اشرب ماء، وخذ دش قصير قبل أي قرار.", "بعد الصلاة": "اربط ما بعد الصلاة بدعاء قصير + كوب ماء + خروج للهواء بدل التدخين.", "القيادة": "نظّف السيارة من أي بقايا، ضع علكة/ماء بجانبك، وغيّر مسارك المعتاد لأول أسبوع.",
};
...
export function buildQuitPlan(intake: QuitPlanIntake, score: QuitPlanScore): QuitPlanJSON {
  ...
  const goalText =
    intake.goal === "quit_full"? "هدفك الآن هو الإقلاع الكامل. سنبني الخطة حول تاريخ بداية واضح، إدارة المحفزات، خطة رغبة شديدة، ومتابعة منتظمة. (WHO 2024; USPSTF 2021)": intake.goal === "reduce_first"? "هدفك الآن هو التقليل أولًا كخطوة نحو الإقلاع. سنركّز على تقليل عدد المرات، تحديد أوقات الاستخدام، وبناء بدائل عملية لكل محفز. (WHO 2024; NCI PDQ)": intake.goal === "not_ready_now"? "هدفك الآن ليس الإقلاع الكامل فورًا. الخطة ستركّز على تقليل الضرر، فهم المحفزات، وتجهيز خطوة صغيرة دون ضغط أو لوم. (WHO 2024; NCI PDQ)": "هدفك الآن هو فهم وضعك. حتى لو لم تكن جاهزًا للإقلاع الكامل اليوم، يمكن البدء بخطوة واضحة مثل فهم المحفزات أو تحديد موعد قريب. (WHO 2024; NCI PDQ)";
  ...
  return {
    ...
    craving_rescue: [
      "أجّل القرار 10 دقائق.", "غيّر مكانك فورًا.", "اشرب كوب ماء بارد ببطء.", "تنفس ببطء لمدة 60 ثانية (شهيق 4، حبس 4، زفير 6).", "اشغل يدك بشيء آخر (علكة خالية من السكر، قلم، مفاتيح).", "تواصل مع شخص داعم برسالة قصيرة.", "افتح أقلع وراجع خطتك.",
    ],
    craving_rescue_citation: "(CDC 2024)",
    first_24h: [
      "أبعد كل منتجات التدخين/النيكوتين من المنزل والسيارة والعمل.", "حدّد أصعب موقفين متوقعين اليوم، وجهّز بديلًا لكل واحد منهما.", "جهّز بدائل في متناول اليد (ماء، علكة، سناك صحي).", "أخبر شخصًا داعمًا واحدًا بقرارك اليوم.", "خطّط لخطة رغبة شديدة واحفظ خطوات الإنقاذ السبع.", "احفظ رابط أقلع وافتحه عند الحاجة.",
    ],
    first_7d: [
      "اليوم الأول: ركّز على البيئة وتجنّب المحفزات المباشرة، ونم مبكرًا.", "اليوم 2–3: راقب الرغبات وسجّل أوقات الضعف ومدتها.", "اليوم 4–7: عدّل الخطة حسب أكثر محفز تكرّر، وكافئ نفسك بنهاية اليوم 7.", "احتفظ بمذكرة قصيرة: متى جاءت الرغبة، وما الذي ساعد.", "حرّك المال الذي كنت تنفقه إلى حصّالة أو تحويل شهري.", "إذا حدث رجوع: لا تحكم على نفسك، عُد للخطة من نفس النقطة.",
    ],
    follow_up_28d: [
      "الأسبوع الأول: تثبيت البداية وإدارة الانسحاب وبيئة آمنة.", "الأسبوع الثاني: تعديل خطة المحفزات بناءً على التجربة الواقعية.", "الأسبوع الثالث: تقوية الدعم (شخص الدعم، صيدلي، مختص عند الحاجة).", "الأسبوع الرابع: تثبيت العادات الجديدة، مراجعة الخطة، وضع هدف لـ 3 أشهر القادمة.",
    ],
    follow_up_28d_citation: "(WHO 2024; USPSTF 2021)",
    relapse_plan: [
      "الرجوع للاستخدام لا يعني أن الخطة فشلت.", "اسأل نفسك: ما المحفز؟ ماذا حدث قبلها؟ وما التعديل المطلوب قبل المرة القادمة؟", "أعد ضبط تاريخ الإقلاع خلال 24–72 ساعة.", "إذا تكرّر الرجوع، اطلب دعمًا مهنيًا (صيدلي/طبيب/مختص).",
    ],
    relapse_plan_citation: "(WHO 2024; NCI PDQ)",
    support_person_plan: intake.support_person?.name
      ? [
          `شخص الدعم: ${intake.support_person.name}${intake.support_person.relation ? ` (${intake.support_person.relation})` : ""}.`, "اتفق معه على رسالة واحدة محفّزة عند الرغبة الشديدة.", "لا يفرض عليك، ولا يلومك إذا حدث رجوع.",
        ]
      : [
          "اختر شخصًا واحدًا تثق به (صديق/قريب/زميل).", "أخبره بقرارك واطلب رسالة دعم قصيرة عند الحاجة.", "لا حاجة لأي ضغط أو مراقبة، فقط حضور إنساني.",
        ],
    pharmacy_discussion: {
      intro:
      ...
      nrt_options: [
        "لصقات النيكوتين", "علكة النيكوتين", "أقراص/مصّات النيكوتين", "بخاخ النيكوتين إن كان متاحًا محليًا", "مستنشق النيكوتين إن كان متاحًا محليًا",
      ],
      nrt_details: [
        {
      ...
      prescription_options: [
        "Varenicline / Champix — يُسأل عنه الطبيب أو الصيدلي.", "Bupropion SR — يُسأل عنه الطبيب أو الصيدلي.",
      ],
      ...
      important_notes: [
        "أقلع لا يصف أدوية ولا يحدد جرعات ولا يحدد مدة الاستخدام.", "اختيار المنتج أو الجرعة المناسبة يحتاج مراجعة صيدلي أو طبيب.", "أبلغ الصيدلي/الطبيب بأي حالة صحية، حمل أو رضاعة، أو أدوية أخرى تستخدمها.",
      ],
      closing:
    ...
    when_to_seek_help: [
      "الحمل أو الرضاعة.", "العمر أقل من 18 سنة.", "أمراض قلب، ضغط، سكري، أو ربو شديد.", "تاريخ نوبات أو تشنجات.", "قلق أو اكتئاب شديد، أو أفكار إيذاء النفس.", "استخدام أدوية متعددة.", "أعراض انسحاب شديدة أو مستمرة.", "رغبة في استخدام دواء أو بديل نيكوتين.", "فشل محاولات إقلاع متكررة.", "استخدام أكثر من منتج نيكوتين في نفس الوقت.",
    ],
    ...
    followup_schedule: [
      "متابعة بعد 24 ساعة.", "متابعة بعد 3 أيام.", "متابعة بعد 7 أيام.", "متابعة بعد 14 يوم.", "متابعة بعد 28 يوم.",
    ],
    contact: {
