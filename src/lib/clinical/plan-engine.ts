...
function buildTimeline(strategy: QuitStrategy, nickname: string): PlanSection[] {
  ...
  const quit_day: PlanSection = {
    ...
    items: [
      ...
      "أخبر من حولك أن اليوم هو يوم إقلاعك حتى لا يُعرض عليك التدخين.",
  ...
  const first_24_hours: PlanSection = {
    ...
    items: [
      "الرغبات في هذه الفترة متقاربة وقصيرة؛ كل رغبة عادةً تمر خلال دقائق.",
      "طبّق قاعدة الـ4 دقائق مع كل رغبة: تأجيل + ماء + تنفس عميق + انشغال.",
      "قلّل المواقف عالية الخطورة اليوم قدر ما تستطيع بدل مواجهتها كلها.",
      "نم مبكرًا إن أمكن؛ التعب يرفع احتمال الاستسلام للرغبة.",
    ],
  };
  ...
  const hours_24_to_72: PlanSection = {
    ...
    items: [
      "أعراض الانسحاب عادةً تكون أشد في هذه الفترة لدى كثير من الناس: عصبية، أرق، صعوبة تركيز. هذا شائع ومؤقت.",
      "قلّل الكافيين تدريجيًا إذا لاحظت زيادة في التوتر أو صعوبة النوم.",
      "أكثر من شرب الماء ووزّع وجبات خفيفة على اليوم.",
      "لا تتخذ قرارات كبيرة أو نقاشات متوترة في هذه الأيام إن أمكن تأجيلها.",
    ],
  };
  ...
  const days_4_to_7: PlanSection = {
    ...
    title_en: "Days 4–7",
    items: [
      "غالبًا تبدأ حدة الأعراض بالتراجع تدريجيًا، لكن المحفزات الموقفية تبقى قوية.",
      "امشِ 10 دقائق مرتين يوميًا — النشاط الخفيف يخفف الرغبة لدى كثيرين.",
      "راجع محفزاتك المكتوبة وأضف خطة مضادة لأي محفز فاجأك هذا الأسبوع.",
      "احتفل بنهاية الأسبوع الأول بمكافأة صغيرة غير مرتبطة بالتدخين.",
    ],
  };
  ...
  const weeks_2_to_4: PlanSection = {
    ...
    title_en: "Weeks 2–4",
    items: [
      "الرغبات تصير أقصر وأقل تكرارًا عادةً، لكنها أشد ارتباطًا بالمواقف والأشخاص.",
      "غيّر مسار طريقك، ومكان جلوسك، ووقت استراحتك.",
      "أكبر خطر في هذه المرحلة هو الثقة الزائدة: «سيجارة وحدة ما تضر» فخ حقيقي.",
      "ابدأ نشاطًا بدنيًا منتظمًا 3 مرات أسبوعيًا إذا كان مناسبًا لحالتك.",
    ],
  };
  ...
  const months_2_to_3: PlanSection = {
    ...
    title_en: "Months 2–3",
    items: [
      "كثير ممن يقلعون يلاحظون تحسنًا في التنفس والسعال خلال هذه الفترة؛ يختلف ذلك من شخص لآخر.",
      "راجع قائمة محفزاتك: أي محفز ما زال يزعجك؟ اكتب له خطة مخصصة.",
      "انتبه لتغيّر الوزن أو الشهية وعالجها بالنشاط والوجبات المنتظمة لا بالحرمان.",
      "ثبّت روتينًا أسبوعيًا لمراجعة تقدمك.",
    ],
  };
  ...
  const months_4_to_6: PlanSection = {
    ...
    items: [
      ...
      "المناسبات الاجتماعية والسفر من أخطر مواقف هذه المرحلة — جهّز ردك المسبق.",
      "أعد قراءة أسبابك الثلاثة كل شهر.",
    ],
  };
  ...
  const months_7_to_12: PlanSection = {
    ...
    items: [
      "احتمال العودة للتدخين ينخفض عادةً، لكنه لا ينعدم، خصوصًا مع الضغوط الكبيرة.",
      "احتفظ بخطة التعامل مع الزلّة في جوالك حتى لو ما احتجتها.",
      "حدد شخص دعم واحد تتواصل معه عند أي ضغط مفاجئ.",
    ],
  };
  ...
  const after_one_year: PlanSection = {
    ...
    items: [
      "الأدلة تشير إلى أن الإقلاع المستمر يقلّل مخاطر صحية عديدة مقارنة بالاستمرار في التدخين؛ المقدار يختلف حسب الشخص وتاريخه الصحي.",
      "أكثر أسباب العودة بعد سنة: أزمة مفاجئة، أو «تجربة سيجارة واحدة» في مناسبة.",
  ...
  const long_term_maintenance: PlanSection = {
    ...
    items: [
      "اعتبر نفسك في وضع «محافظة دائمة»: لا سيجارة تجريبية، ولا استثناءات في السفر أو الأزمات.",
      "أعد تقييم مواقفك عالية الخطورة عند أي تغيّر كبير في حياتك (عمل جديد، فقد، انتقال).",
      "إذا حدثت زلّة بعد سنوات، ارجع لخطة الزلّة فورًا بدل الانتظار.",
      "شارك تجربتك مع شخص يفكر في الإقلاع — الاستمرارية تقوى بالمساندة.",
    ],
  };
