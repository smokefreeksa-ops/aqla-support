...
const TRIGGER_PLAN_TEMPLATES: Partial<Record<TriggerKey, PlanSection>> = {
  coffee: {
    ...
    steps: [
      ...
      "غيّر الكوب.",
      "اشرب ماء معها.",
      "بعد القهوة، امشِ دقيقتين.",
      "سجّل كل قهوة ناجحة.",
      "لا تشربها مع مدخنين أول أسبوعين.",
    ],
  ...
  car: {
    ...
    steps: [
      "نظّف السيارة من الرائحة.",
      "أزل كل الولاعات والعلب.",
      "ضع ماء وعلكة داخل السيارة.",
      "جهّز محتوى صوتي مخصص للقيادة.",
      "لا قهوة داخل السيارة أول أسبوع إذا كانت محفزة.",
      "إذا اشتدت الرغبة: توقف في مكان آمن وامشِ.",
    ],
  ...
  after_meal: {
    title: "خطة بعد الأكل",
    steps: [
      "انهض فورًا بعد الأكل.",
      "اغسل فمك أو أسنانك.",
      "امشِ خمس دقائق.",
      "لا تجلس في مكان التدخين المعتاد.",
      "جهّز ماء أو علكة خالية من السكر.",
    ],
  ...
  stress: {
    ...
    steps: [
      "عند التوتر: لا قرار أول 60 ثانية.",
      "اخرج من المكان.",
      "تنفس 10 مرات ببطء.",
      "امشِ 5 دقائق.",
      "أرسل رسالة: عندي رغبة، أحتاج دعم.",
      "عالج المشكلة بعد الهدوء.",
    ],
  ...
  shisha_session: {
    ...
    steps: [
      "لا تحضر جلسات شيشة أول 30 يومًا.",
      "اقترح جلسة بديلة لصديق واحد.",
      "لا تمسك الخرطوم حتى للمزاح.",
      "جهّز جملة: أنا مقلع، لا أجرب حتى نفس.",
      "استبدل الويكند بنشاط بلا دخان.",
    ],
  ...
  social: {
    ...
    steps: [
      "اختر مجلسًا واحدًا تتجنبه أول شهر.",
      "أخبر صديقًا واحدًا أنك مقلع.",
      "اجلس بعيدًا عن المدخنين في أي مجلس.",
      "جهّز جملة قصيرة للرفض دون حرج.",
      "اقترح نشاطًا بديلًا للجلسة (مشي، قهوة في مكان مفتوح).",
    ],
...
const VAPE_PLAN: PlanSection = {
  ...
  steps: [
    "سجّل عدد مرات الاستخدام أو أوقاته يوميًا.",
    "امنع الاستخدام في السرير والسيارة والحمام.",
    "حدد نوافذ استخدام مؤقتة إذا لم تكن جاهزًا للتوقف.",
    "ناقش خطة علاج أو تقليل مع مختص.",
    "الهدف النهائي: لا اعتماد على النيكوتين.",
  ],
...
function buildBasePlan(a: EngineAnswers, dep: string, ready: string): PlanSection {
  if (dep === "high") {
    return {
      ...
      steps: [
        "لا تواجه النيكوتين وحدك. ناقش العلاج المناسب مع طبيب/صيدلي/عيادة إقلاع.",
        "لا تبدأ يوم الإقلاع دون خطة انسحاب واضحة.",
        "استخدم متابعة قريبة (شخص داعم + مختص).",
        "اجعل أول استخدام صباحي هو الهدف الأول الذي تكسره.",
        "تجنب المجالس عالية الخطورة أول شهر كامل.",
      ],
    };
  }
  if (ready === "ready_now" || ready === "wants_but_low_confidence") {
    return {
      ...
      steps: [
        "حدد تاريخ الإقلاع خلال 7–14 يومًا.",
        "قبل التاريخ بـ 7 أيام: سجّل كل استخدام.",
        "قبل التاريخ بـ 5 أيام: نظّف السيارة والبيت من الأدوات.",
        "قبل التاريخ بـ 3 أيام: أخبر شخصًا داعمًا.",
        "قبل التاريخ بيوم: جهّز بطاقة الرغبة وسلة البدائل.",
        "يوم الإقلاع: لا تختبر نفسك في مجلس دخان.",
        "أول 72 ساعة: اتبع الخطة اليومية حرفيًا.",
      ],
    };
  }
  return {
    ...
    steps: [
      "اجعل البيت والسيارة بلا دخان.",
      "سجّل استخدامك 7 أيام.",
      "اختر محفزًا واحدًا فقط لتغييره.",
      "جرّب 24 ساعة بلا نيكوتين أو تأخير أول استخدام.",
      "اقرأ أسبابك الشخصية كل صباح.",
      "احجز استشارة دون التزام إذا أمكن.",
    ],
  };
}
...
export function buildPlan(a: EngineAnswers): EngineResult {
  ...
  const result: EngineResult = {
    ...
    seventy_two_hour_plan: [
      "لا مجالس دخان.",
      "لا سيارة فيها أدوات تدخين.",
      "لا قهوة في المكان القديم.",
      "بعد الأكل: حركة وتنظيف فم.",
      "عند الرغبة: استخدم بطاقة الطوارئ.",
      "متابعة قصيرة مع شخصك الداعم بعد 3 أيام.",
    ],
    ...
    referral_message: referral
      ? "بناءً على إجاباتك، الأفضل أن تتواصل مع طبيب أو صيدلي أو عيادة إقلاع قبل اختيار أي علاج دوائي. هذا لا يعني أن حالتك صعبة فقط، بل يعني أن خطتك تستحق أن تكون آمنة وقوية."
      : "لا يظهر من إجاباتك ما يستدعي إحالة عاجلة، لكن يمكنك دائمًا مناقشة خيارات الدعم والعلاج مع صيدلي أو طبيب إذا رغبت.",
