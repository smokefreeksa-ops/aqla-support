...
const CHARTER_AR = [
  "أنا لست وحدي.", "الإقلاع ليس فشلًا متكررًا، بل تجربة تتراكم.", "أساند، ولا أحكم.", "أتعلّم قبل أن أنصح.", "خصوصيتي وخصوصية غيري أمانة.", "مدينتي تتنفّس معي.",
];
const CHARTER_EN = [
  "I am not alone.", "Quitting is not repeated failure. It is accumulating experience.", "I support without judging.", "I learn before I advise.", "My privacy, and others' privacy, is a trust.", "My city breathes with me.",
];
...
const COPING_AR = [
  "أخّر الرغبة 5 دقائق", "اشرب ماء", "غيّر مكانك", "خذ نفسًا هادئًا", "تواصل مع شخص داعم", "استخدم مدرب اللحظة", "ابدأ تقييم أقلع",
];
const COPING_EN = [
  "Delay the craving 5 minutes", "Drink water", "Change your location", "Take a slow breath", "Reach a supportive person", "Use the moment coach", "Start the Aqla assessment",
];

const MAJLIS_PHRASES = [
  "يعطيك العافية، اليوم بحاول أخفف.", "تسلم، خليني على القهوة اليوم.", "والله ناوي أبدأ أقلل، ادعمني.", "ما قصرت، بس اليوم بجرّب أتركها.", "خلنا نغيرها اليوم، قهوة وسوالف بدون دخان.",
];
...
function Inner() {
  ...
  const shareCharter = (channel: "whatsapp"| "x") => {
    const text = isAr
      ? "وقّعت ميثاق أقلع. انضم إلينا: ": "I signed the Aqla Charter. Join us: ";
      ...
      <main className="mx-auto max-w-5xl px-4 py-10 sm:py-14">
        ...
        <section className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-3xl text-[15px] leading-8 text-muted-foreground">
            {isAr
              ? "أقلع ليست مجرد منصة. إنها مساحة يشارك فيها الأفراد والمدن والمتطوعون في نشر الوعي، دعم الإقلاع، وبناء أثر مجتمعي قابل للقياس دون عرض أي بيانات شخصية.": "Aqla is more than a platform. It is a space where people, cities, and volunteers participate in awareness, cessation support, and measurable community impact without displaying personal data."}
          </p>
        </section>
        ...
        <section className="mt-12">
          <Card className="rounded-3xl border-0 p-6 sm:p-8 shadow-elegant card-gradient">
            ...
            <p className="mt-4 text-xs leading-6 text-muted-foreground">
              {isAr
                ? "مؤشر أقلع يعكس التفاعل المجتمعي مع المنصة، ولا يمثل نتيجة طبية أو معدل إقلاع مؤكد.": "The Aqla Index reflects community engagement with the platform. It is not a medical outcome or confirmed quit rate."}
            </p>
          </Card>
        </section>
            ...
            {signed ? (
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                ...
                <label className="sm:col-span-2 flex items-start gap-2 text-xs text-muted-foreground">
                  ...
                  <span>
                    {isAr
                      ? "أوافق على عرض اسمي (أو لقبي) ضمن أعضاء الميثاق علنًا. (اختياري)": "I consent to my name/nickname being shown publicly among Charter members. (Optional)"}
                  </span>
                </label>
                <div className="sm:col-span-2">
                  ...
                  <p className="mt-2 text-[11px] text-muted-foreground">
                    {isAr
                      ? "لا نطلب بريدك أو رقم جوالك للتوقيع على الميثاق.": "We do not collect email or phone for charter signing."}
                  </p>
                </div>
              </div>
            )}
        ...
        <section className="mt-12">
          <Card className="rounded-3xl border-0 p-6 sm:p-8 shadow-elegant card-gradient">
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              {isAr
                ? "عبارات مهذبة بالعربية الطبيعية للاعتذار عن التدخين في المجلس أو الديوانية، دون إحراج أو حكم.": "Culturally respectful Arabic phrases to decline smoking in majlis/diwaniya without judgment or embarrassment."}
            </p>
        ...
        <p className="mt-10 text-center text-[11px] text-muted-foreground">
          {isAr
            ? "العدّادات تجميعية وعامة فقط. لا نعرض بيانات شخصية أو نتائج صحية فردية.": "All counters are aggregate and public-safe. No personal data or individual health outcomes are shown."}
        </p>
      </main>
    </div>
  );
