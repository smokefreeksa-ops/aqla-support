...
import { Sparkles, Wind, ChevronLeft, MessageCircle } from "lucide-react";
  ...
  return (
    <div dir={dir} className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
        ...
        <Link
          to="/quit-chat"
          className="mt-6 block rounded-2xl border border-teal-500/30 bg-gradient-to-br from-teal-950/40 via-slate-900 to-slate-900 p-5 hover:border-teal-400/60 transition group"
        >
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[11px] font-bold tracking-widest text-teal-400">جديد · NEW</div>
              ...
              <p className="text-xs text-foreground/70 mt-1 leading-6">
                {isAr
                  ? "تجربة محادثة سريرية: تقييم Fagerström، قياس الاستعداد، وبناء خطة START خطوة بخطوة."
                  : "Clinical chat: Fagerström test, readiness scale, and step-by-step START plan."}
              </p>
            </div>
            <ChevronLeft className="h-5 w-5 text-teal-400 group-hover:-translate-x-1 transition" />
          </div>
        </Link>
