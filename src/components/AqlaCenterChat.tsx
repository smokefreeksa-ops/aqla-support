...
export function AqlaCenterChat({ centerType }: { centerType: CenterType }) {
  ...
  if (!status?.enabled) {
    return (
      ...
      >
        {isRTL
          ? "المساعد التفاعلي غير متاح حاليًا. يمكنك تصفّح الموارد أدناه.": "The interactive assistant is currently unavailable."}
      </div>
    );
  }
    ...
    >
      ...
      >
        ...
        <p className="mt-1.5 text-[11px] text-muted-foreground">
          {isRTL
            ? "معلومات تثقيفية فقط — ليست تشخيصًا أو علاجًا.": "Educational info only — not diagnosis or treatment."}
        </p>
      </form>
    </div>
  );
